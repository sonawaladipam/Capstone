...
const CustomerDetails = () => {
  ...
  const handleCustomerClick = async (customer) => {
    const specificName = 'John Doe'; // Replace with the specific name you want to compare
    if (customer.name.toLowerCase() === specificName.toLowerCase()) {
      try {
        const response = await axios.get(`http://localhost:3000/api/appointments/by-customer-name?customerName=${encodeURIComponent(customer.name)}`);
        setAppointments(response.data);
        setSelectedCustomer(customer);
        setIsModalOpen(true);
      } catch (error) {
      ...
      console.log('Name does not match the comparison value');
    }
  };
